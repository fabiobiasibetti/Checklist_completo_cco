          ...
          {isLoading ? (
            ...
            >
                    ...
                    <div className="flex justify-between items-start">
                        ...
                        {record.hasPartial && (
                            <div 
                                ...
                                className="text-[9px] font-black text-blue-500 hover:text-blue-600 underline whitespace-nowrap ml-2 cursor-pointer transition-colors"
                            >
              ...
              <div className="flex items-center gap-2 px-2 py-1 bg-black/5 dark:bg-white/5 rounded-lg">
                  ...
                  {record.resetBy === 'Salvamento automático (10:00h)' && (
                      <span className="text-[8px] bg-amber-500 text-white px-1.5 py-0.5 rounded ml-auto">AUTO</span>
                  )}
              </div>
            </button>
          ))}
          ...
          <>
            <div className="p-4 bg-gray-50 dark:bg-slate-800 border-b border-gray-200 dark:border-slate-700 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
               <div className="flex items-center gap-4">
                  <div className={`w-12 h-12 ${viewingPartial ? 'bg-amber-500' : 'bg-blue-600'} rounded-2xl flex items-center justify-center text-white shadow-lg transition-colors`}>
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-3 mt-1">
                        ...
                        <span className="text-[10px] flex items-center gap-1 text-slate-500 font-bold uppercase">
                            <Calendar size={10} /> {displayTimestamp(viewingPartial && selectedRecord.partialRecord ? selectedRecord.partialRecord.timestamp : selectedRecord.timestamp)}
                        </span>
                    </div>
                  </div>
               </div>
                  ...
                  {selectedRecord.hasPartial && (
                      <button 
                        ...
                        className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase border transition-all flex items-center gap-2 shadow-sm ${viewingPartial 
                            ...
                            : 'bg-amber-100 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400 border-amber-200 dark:border-amber-800 hover:bg-amber-200'}`}
                      >
                          {viewingPartial ? (
                              <><Eye size={14}/> Ver Snapshot Principal</>
                          ) : (
                              <><Sparkles size={14}/> Ver Salvamento Parcial (10:00h)</>
                          )}
                      </button>
                  )}
                  <div className="px-3 py-1 bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400 rounded-full text-[10px] font-black uppercase border border-green-200 dark:border-green-800 flex items-center">
                      Snapshot Recuperado
                  </div>
               </div>
            </div>
            ...
            <div className="flex-1 overflow-auto bg-slate-50 dark:bg-slate-950 scrollbar-thin">
                <table className="w-full border-collapse bg-white dark:bg-slate-900 text-[10px]">
                  <thead className="sticky top-0 z-20 bg-slate-800 dark:bg-slate-950 text-white shadow-lg">
                    <tr>
                      <th className="p-2 text-left min-w-[200px] w-[30%] border-r border-slate-700 dark:border-slate-800 sticky left-0 bg-slate-800 dark:bg-slate-950 z-30 shadow-[4px_0_8px_-4px_rgba(0,0,0,0.3)] font-black uppercase tracking-widest text-[9px]">AÇÃO / TAREFA</th>
                      {getAllLocations(currentTasksToDisplay).map(loc => (
                        <th key={loc} className="p-1 text-center min-w-[45px] border-r border-slate-700 dark:border-slate-800 font-bold">
                  ...
                  <tbody>
                    {(() => {
                        ...
                        if (categories.length === 0) {
                            return <tr><td colSpan={locs.length + 1} className="p-8 text-center text-slate-400 italic">Nenhum dado disponível neste snapshot.</td></tr>;
                        }
                        ...
                        return categories.map(category => {
                            ...
                            return (
                                <React.Fragment key={category}>
                                    <tr 
                                        className="bg-slate-600 dark:bg-slate-800 text-white font-bold uppercase tracking-widest cursor-pointer hover:bg-slate-700 dark:hover:bg-slate-700 transition-colors text-[11px] py-1"
