...
const TaskManager: React.FC<TaskManagerProps> = ({ 
  ...
  const handleOpenResetModal = async () => {
    ...
    try {
        ...
        if (token) {
            ...
            setResetResponsible(users.length === 1 ? users[0] : '');
        }
  ...
  const handleUpdateStatus = async (taskId: string, location: string, status: OperationStatus) => {
    ...
    try {
      await SharePointService.updateStatus(currentUser.accessToken, {
        ...
        Title: `MATRIZ_${taskId}_${location}`
      });
    } catch (err: any) {
  ...
  const handlePaintRow = async (taskId: string) => {
    ...
    setIsUpdating(true);
    try {
      await Promise.all(locations.map(loc => SharePointService.updateStatus(currentUser.accessToken!, {
        DataReferencia: new Date().toISOString(),
        TarefaID: taskId,
        OperacaoSigla: loc,
        Status: activeTool,
        Usuario: currentUser.name,
        Title: `MATRIZ_${taskId}_${loc}`
      })));
    } catch (err: any) {
  ...
  const handleResetChecklist = async () => {
    ...
    setIsUpdating(true);
    try {
        // 1. Salva SNAPSHOT no histórico (com todos os dados atuais)
        ...
        // 2. Reseta apenas as CÉLULAS DO USUÁRIO na matriz permanente
        ...
        alert("Checklist salvo no histórico e resetado com sucesso!");
    } catch (error: any) {
  ...
  const onCellInteraction = (taskId: string, loc: string, forcedStatus?: OperationStatus) => {
    ...
    if (currentStatus !== nextStatus) handleUpdateStatus(taskId, loc, nextStatus);
  };
  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-slate-900 rounded-2xl border dark:border-slate-800 shadow-sm overflow-hidden relative transition-colors duration-500">
      <div className="px-4 py-3 border-b dark:border-slate-800 flex flex-col xl:flex-row justify-between items-center bg-gray-50/80 dark:bg-slate-800/80 backdrop-blur-md gap-3 shrink-0 z-50">
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            <div className="p-2 bg-blue-600 rounded-lg text-white shadow-lg shadow-blue-500/20"><Activity size={20} /></div>
          ...
          {isUpdating ? (
            <div className="flex items-center gap-2 text-[10px] text-blue-500 animate-pulse font-black uppercase tracking-widest"><Loader2 size={12} className="animate-spin"/> Gravando</div>
          ) : (
            <div className="flex items-center gap-2 text-[10px] text-green-500 font-bold uppercase tracking-widest"><ShieldCheck size={12}/> Otimizado</div>
          )}
        </div>
        ...
        <div className="flex flex-wrap items-center justify-center gap-3">
          <div className="flex items-center gap-2 bg-white dark:bg-slate-950 px-3 py-1.5 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
            <div className="flex items-center gap-2 text-[10px] font-bold text-slate-400 uppercase tracking-tight mr-1">
              <PaintBucket size={14} className={activeTool ? 'text-blue-500' : 'text-slate-400'} /> {activeTool || 'Pincel'}
            </div>
            <div className="flex items-center gap-1">
              {Object.entries(STATUS_CONFIG).map(([key, cfg]) => (
                <button key={key} onClick={() => setActiveTool(activeTool === key ? null : key as OperationStatus)} 
                  className={`w-7 h-7 rounded-lg font-black text-[9px] transition-all duration-200 border flex items-center justify-center relative group ${cfg.color} ${activeTool === key ? 'ring-2 ring-offset-2 ring-blue-500 scale-110 z-10' : 'opacity-80 hover:opacity-100'}`} title={`${cfg.desc} [${cfg.shortcut}]`}>
                  {cfg.label}
          ...
          <div className="flex items-center gap-1">
            <button onClick={handleOpenResetModal} className="flex items-center gap-2 px-3 py-2 bg-amber-50 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400 rounded-xl hover:bg-amber-100 transition-all border border-amber-100 shadow-sm" title="Salvar e Resetar Checklist">
              <RotateCcw size={18} /> <span className="text-xs font-bold hidden sm:inline">Resetar Checklist</span>
            </button>
                ...
                <div className="bg-amber-700 dark:bg-slate-800 text-white p-4 flex justify-between items-center">
                    <div className="flex items-center gap-2">
                        ...
                        <div>
                            ...
                            <p className="text-[10px] text-amber-200">Snapshot será salvo no histórico permanente</p>
                        </div>
                    </div>
                ...
                <div className="p-6 bg-gray-50 dark:bg-slate-900">
                    <div className="mb-6">
                        ...
                        <div className="relative">
                            {isLoadingUsers ? (
                                <div className="flex items-center gap-2 p-3 bg-white border rounded-xl text-slate-400 text-sm italic"><Loader2 size={16} className="animate-spin" /> Buscando...</div>
                            ) : registeredUsers.length > 0 ? (
                                <select value={resetResponsible} onChange={(e) => setResetResponsible(e.target.value)} className="w-full p-3 border rounded-xl bg-white text-sm focus:ring-2 focus:ring-amber-500 outline-none appearance-none font-bold">
                                    <option value="">Selecione seu nome...</option>
                                    {registeredUsers.map(name => <option key={name} value={name}>{name}</option>)}
                                </select>
                            ) : (
                                <div className="p-4 bg-red-50 border border-red-100 rounded-xl text-red-600 text-xs flex flex-col gap-2">
                                    <div className="flex items-center gap-2 font-bold uppercase"><AlertCircle size={16} /> Não autorizado</div>
                                    <p>E-mail <b>{currentUser.email}</b> não encontrado na lista 'Usuarios_cco'.</p>
                                </div>
                            )}
                        </div>
                    </div>
                    <div className="flex gap-3">
                        <button onClick={() => setIsResetModalOpen(false)} className="flex-1 py-3 bg-gray-200 text-gray-700 font-bold rounded-xl">Cancelar</button>
                        <button onClick={handleResetChecklist} disabled={!resetResponsible.trim() || isUpdating || isLoadingUsers} className="flex-[2] py-3 bg-amber-600 text-white font-bold rounded-xl shadow-lg flex items-center justify-center gap-2 disabled:opacity-50">
            ...
            <table className={`min-w-full border-separate border-spacing-0 select-none ${compact ? 'text-[10px]' : 'text-[11px]'}`}>
              <thead className="sticky top-0 z-[40]">
                <tr className="bg-blue-900 dark:bg-blue-950 text-white shadow-xl">
                  <th className="p-3 border-r border-blue-800 text-left sticky left-0 bg-blue-900 z-[45] min-w-[350px] font-black uppercase tracking-widest text-[9px]">Ação / Descrição da Tarefa</th>
                  {locations.map(loc => (
                ...
                {(Object.entries(groupedTasks) as [string, Task[]][]).map(([cat, catTasks]) => {
                  ...
                  return (
                    <React.Fragment key={cat}>
                      <tr className={`bg-blue-600 dark:bg-blue-900 text-white transition-colors h-10 group cursor-pointer ${!isComplete && !isCollapsed ? 'opacity-90' : 'hover:bg-blue-700'}`} onClick={() => toggleCategory(cat)}>
                        <td colSpan={locations.length + 1} className="p-0 border-y border-blue-700 sticky left-0 z-30 overflow-hidden">
                          ...
                          <div className="absolute inset-0 px-4 flex items-center justify-between z-10">
                            <div className="flex items-center gap-3">{isCollapsed ? <ChevronRight size={14}/> : <ChevronDown size={14}/>} <span className="text-[10px] font-black uppercase">{cat}</span></div>
                            <span className="text-[9px] font-black bg-black/20 px-2 py-0.5 rounded-lg flex items-center gap-1">{isComplete && <CheckCircle size={10} />} {percent}% {isComplete ? '(OK)' : ''}</span>
                          </div>
                        </td>
                      </tr>
                      {!isCollapsed && catTasks.map(task => (
                        <tr key={task.id} className="bg-white dark:bg-slate-900 border-b border-slate-100 dark:border-slate-800 hover:bg-blue-50/30 transition-colors group">
                          <td className={`p-4 border-r sticky left-0 bg-inherit z-30 shadow-[4px_0_8px_-4px_rgba(0,0,0,0.1)] transition-all ${activeTool ? 'cursor-crosshair hover:bg-blue-50' : ''}`} onClick={() => handlePaintRow(task.id)}>
                            <div className="flex flex-col gap-1.5">
                                <div className="font-bold text-slate-800 dark:text-slate-100 text-[13px] leading-tight">{task.title}</div>
                                {task.description && <div className="text-[11px] font-normal text-slate-500 dark:text-slate-400 leading-snug opacity-90">{task.description}</div>}
                            </div>
                          </td>
                          {locations.map(loc => {
                            ...
                            return (
                              <td key={loc} className="p-0 border-r dark:border-slate-800 h-12 relative" onMouseDown={() => { setIsDragging(true); onCellInteraction(task.id, loc); paintedThisDrag.current.add(`${task.id}-${loc}`); }}
                                ...
                                <div className={`absolute inset-[3px] rounded-lg flex items-center justify-center transition-all duration-200 font-black text-[10px] ${cfg.color} hover:brightness-95 cursor-pointer shadow-sm`}>{cfg.label}</div>
                              </td>
                            );
                          })}
                        </tr>
      ...
      <div className="px-4 py-2 bg-white dark:bg-slate-900 border-t flex justify-between items-center text-[9px] text-slate-400 font-bold uppercase tracking-widest shrink-0">
          <div className="flex items-center gap-2"><div className="w-1.5 h-1.5 rounded-full bg-green-500"></div> Usuário: {currentUser.name}</div>
          <div className="flex items-center gap-2"><HelpCircle size={10} className="text-blue-500"/> Matriz Sincronizada: Updates PATCH ativos</div>
      </div>
    </div>
  );
};
